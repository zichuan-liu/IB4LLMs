...
if not AZURE:
    ...
    NOT_AZURE_URL = YOUR_URL     # URL
    NOT_AZURE_KEY = YOUR_KEY   # KEY
else:
    AZURE_OPENAI_ENDPOINT = YOUR_URL   # URL
    AZURE_OPENAI_KEY = YOUR_KEY       # KEY
...
def result_print(scores_attacked, mode="Duo",file_path=""):
    scores_attacked_np = np.array(scores_attacked[0])
    scores_attacked_np = np.where(scores_attacked_np == None, 5, scores_attacked_np)  
            ...
            try:
                if not AZURE:
                    ...
                    response = client.chat.completions.create(
                        ...
                        top_p=1e-8,temperature=0,max_tokens=4096,frequency_penalty=0,presence_penalty=0,
                    )
...
if __name__ == "__main__":
    ...
    # finish_result.append("../perturb_results/GCG_vib_llama2_space/result.jsonl")

    # import jsonlines
    # for file_path in finish_result:
    #     output_file = file_path.replace("result.jsonl", "result_scores_and_resons.jsonl")
    #     with open(output_file, 'r') as f:
    #         scores = []
    #         for qa_record in jsonlines.Reader(f):
    #             scores.append(qa_record["duo_score"])
    #         print(file_path, "scores: %.3f"%np.mean(scores))
    ...
    parser.add_argument(
        '--file_path',
        type=str,
        default="../perturb_results/PAIR_vib_vicuna_/result.jsonl"
    )
    parser.add_argument(
        '--ez',
        type=bool,
    ...
    finish_result.append(file_path)
    entry = 'def_result'
    data = []
    ...
    result_print(duo_results, mode="Duo", file_path=file_path)
